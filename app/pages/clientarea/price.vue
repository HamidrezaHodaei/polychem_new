<template>
  <div class="h-screen w-screen overflow-hidden relative">
    ...
    <div class="relative z-10 h-full flex flex-col p-6">
      ...
      <header class="bg-gray-200/50 backdrop-blur-lg rounded-3xl px-6 py-3 mb-4 flex items-center justify-between shadow-lg border border-white/30">
        ...
        <nav class="flex items-center gap-6">
          <button v-for="tab in tabs" :key="tab" 
            :class="['px-5 py-2.5 font-medium transition-all text-base relative group']"
      ...
      <!-- Price List Page -->
      <div v-if="activeTab === 'Price List'" class="flex-1 overflow-hidden">
        <div class="grid grid-cols-12 gap-4 h-full">
          ...
          <div class="col-span-8 grid grid-cols-2 gap-4 h-full">
            ...
            <!-- Transit Way -->
            <div class="bg-white/60 backdrop-blur-xl rounded-3xl p-5 shadow-xl border border-white/40 w-full h-auto">
...
export default {
  ...
  methods: {
    handleTabClick(tab) {
      this.activeTab = tab
      if (tab === 'Dashboard') {
        if (this.$router) this.$router.push('/clientarea')
        else console.warn('Router not available')
      }
    },
    getPaymentLabel() {
